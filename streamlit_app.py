...
def generate_llama2_response(model, tokenizer, device, prompt_input, temperature, top_p, max_length):
...
def main():
    ...
    # Sidebar for API token and parameters
    with st.sidebar:
        st.title('MindMate 🧠')
        ...
        st.button('Clear Chat History', on_click=clear_chat_history, key='clear_chat_history_button')
    ...
    if st.session_state.messages[-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                response = generate_llama2_response(model, tokenizer, device, st.session_state.messages[-1]["content"], temperature, top_p, max_length)
